...
for j in range (0, Nt+1):
  for i in range (1,Nx):
    u[i,j] = np.e**(-(np.pi**2)*t[j] ) * np.sin(np.pi*x[i])
...
#izprinte katru desmito u(x,t) liimeni ar analitiskajam vertibam
...
for i in range(1,Nt+1, int(Nt/10)):
  axs[0].plot(u[:,i], 'b--',linewidth = '3')

# sakumnosacijuma likne
axs[0].plot(u[:,0], 'r-', linewidth='3')
